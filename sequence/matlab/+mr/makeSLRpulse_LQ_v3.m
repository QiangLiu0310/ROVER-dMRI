function [rf, gz, gzr, delay] = makeSLRpulse_LQ_v3(flip,varargin)
...
if isempty(parser)
    parser = inputParser;
    parser.FunctionName = 'makeSLRpulse_LQ_v3';
    ...
    addParamValue(parser, 'filterType', 'ls', @isstr); % mt QL
...
%%  Qiang Liu called the dzrf from matlab
% N=256;
[signal ]=dzrf_0903(N,opt.timeBwProduct,ptype);
...
% flip_1 = sum(abs(signal*opt.dwell))*2*pi;
% factor_1=180/flip;
% factor_1_ref=factor_1*2.4 ; % just calculated from the voltage if we want 250 v
% factor_2=180/flip_1;
% sum_real=sum(real(signal));
% sum_imag=sum(imag(signal));
% flip_siemens=opt.dwell*2*pi*sqrt(sum_real.^2+sum_imag.^2);
% factor_siemens=180/flip_siemens;
signal = signal*opt.flipAngle/flip;%QL
...
rf.delay = opt.delay;
% Sequence registration in Pulseq v1.5 expects center/freqPPM/phasePPM fields.
...
if nargout > 1
    ...
    gzr= mr.makeTrapezoid('z', opt.system, 'Area', -area*(1-rf.center/rf.shape_dur)-0.5*(gz.area-area));
