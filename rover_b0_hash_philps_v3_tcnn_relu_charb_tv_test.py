...
`rover_b0_hash_philps_v3_tcnn_relu_charb_tv.py`.

Geometry: 9 depth layers, spa_res 1.4 mm (through-plane), Philips phantom paths.
...
slice, so the through-plane voxel spacing of the output equals spa_res (1.4 mm).
The saved NIfTI z-axis voxel size is set accordingly (--z_spacing_mm, default 1.4).
"""
...
def fix_affine_z_spacing(reference_affine, target_z_spacing_mm=1.4):
...
affine_path = '/scratch/home/ql087/data_bwh/Philips/For_Qiang_phantom/nii_new/preprocess_rover_nii/Affine_nii_1.npy'
...
if os.path.exists(affine_path):
    ...
    print(f"Depth ratio: {z_voxel_size:.6f} / {x_voxel_size:.6f} = {depth_ratio:.6f}")
    print(f"Number of depth layers: {num_depth_layers}")
    print(f"Through-plane spa_res: {spa_res:.6f} mm")
else:
    spa_res = 1.4
    num_depth_layers = 9
    print(f"Could not load reference Affine matrix; using defaults:")
    print(f"  Spatial resolution: {spa_res:.6f} mm")
    print(f"  Number of depth layers: {num_depth_layers}")
...
parser.add_argument('--z_spacing_mm', type=float, default=spa_res,
                    help="NIfTI z-axis voxel size in mm (through-plane output spacing). Default = spa_res (1.4).")
...
print(f"NIfTI header z-spacing: {orig_z_spacing:.6f} mm -> {out_z_spacing:.6f} mm (target {opts.z_spacing_mm} mm)")
...
print(f"\n--- Load image data ---")
print(f"Image path pattern: {args.img_path}")
...
for view_num in range(nv):
    ...
    affine_path = f'/scratch/home/ql087/data_bwh/Philips/For_Qiang_phantom/nii_new/preprocess_rover_nii/Affine_nii_{view_num + 1}.npy'
...
if use_direct_checkpoint_path:
    ...
    output_filename = f'rover_b0_hash_philps_tcnn_relu_charb_tv_{ckpt_basename}_z{opts.z_spacing_mm:.5f}.nii'
else:
    output_filename = (
