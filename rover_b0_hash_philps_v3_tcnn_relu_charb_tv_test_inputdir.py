...
affine_shapes = [np.load(path, mmap_mode='r').shape for path in affine_paths]
if any(shape != (4, 4) for shape in affine_shapes):
    raise ValueError(
        "All affine files must have shape (4, 4). Found shapes: "
        + ", ".join(f"{os.path.basename(path)}={shape}" for path, shape in zip(affine_paths, affine_shapes))
    )
